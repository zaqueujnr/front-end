...
test('Deve renderizar com paginação', async () => {
  const getWorksMock = jest
    .fn()
    .mockResolvedValueOnce({ works: worksFirstResponse, total: 5, totalPages: 2 })
    .mockResolvedValueOnce({ works: worksSecondResponse, total: 3, totalPages: 2 });
...
test('Deve filtrar por palavra chave com paginação', async () => {
  const getWorksMock = jest
    ...
    .mockResolvedValueOnce({ works: worksFirstResponse, total: 8, totalPages: 2 })
    .mockResolvedValueOnce({ works: worksSecondResponse, total: 3, totalPages: 2 });
...
test('Deve filtrar por palavra chave depois remover a pesquisa', async () => {
  const getWorksMock = jest
    .fn()
    .mockResolvedValueOnce({ works: [] })
    .mockResolvedValueOnce({ works: worksSecondResponse })
    .mockResolvedValueOnce({ works: worksFirstResponse });
