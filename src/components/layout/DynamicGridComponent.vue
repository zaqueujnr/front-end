...
watch(() => props.totalList, (newValue) => {
...
const onScroll = () => {
    ...
    const viewPortBottom = container.scrollTop + container.clientHeight >= container.scrollHeight - 10
    if (!props.loading && viewPortBottom)
        emit('loadMore')
...
<template>
    <div ref="dynamicGridContainer" class="dynamic-grid" :class="`lg:grid-cols-${props.cols} grid-cols-6`" @scroll="onScroll">
        <slot></slot>
    </div>
</template>
